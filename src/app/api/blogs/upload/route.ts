...
function sanitizeFilename(filename: string): string {
  ...
  return `${sanitized || 'image'}-${Date.now()}${ext.toLowerCase()}`;
}
...
export async function POST(request: Request) {
  try {
    ...
    // 7. Gửi tệp lên Supabase Storage qua REST API
    ...
    if (!uploadRes.ok) {
      ...
      if (uploadData.error === 'Bucket not found' || (uploadData.message && uploadData.message.includes('not found'))) {
        return NextResponse.json({ 
          error: 'Chưa tạo bucket "blogs" chế độ Public trên Supabase Storage. Vui lòng tạo bucket trước.' 
        }, { status: 500 });
      }
